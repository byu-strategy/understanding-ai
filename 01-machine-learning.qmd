...
In the next section, we will introduce a training algorithm called **gradient descent**, which is designed to iterate over many possible combinations of values for $w_0$ and $w_1$ in order to find (i.e. *learn*) the values that minimize the loss. In order to have a solid understanding of how gradient descent will do it's job we need to first spend a bit more time with the loss function to understand it's properties. A picture often is worth 1000 words so we will start there. 

Recall that we can plot a single variable function on a two dimensional $x$-$y$ coordinate system as we did earlier when drawing the regression equation as a line. The loss function, however, is a function of two variables ($\mathcal{L}(w_0, w_1)$) which means we need three dimensions to visualize it. We can plot $\mathcal{L}$ on the vertical axis (often notated as the $z$ axis in other contexts), $w_0$ on what is typically called the x-axis, and $w_1$ for the $y$ axis. This is illustrated in the plot below:
